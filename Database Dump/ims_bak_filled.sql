...
INSERT INTO `budget` VALUES (4,'2020-12-01','2020-12-31',1234.23,6221.34,4987.110000000001,00444444444),(5,'2020-01-01','2020-01-31',3212.56,2332.75,-879.81,00444444444),(6,'2020-02-01','2020-02-28',2311.34,3523.83,1212.4899999999998,00444444444);
...
INSERT INTO `incoming_goods` VALUES (00000000013,00628352190,'2020-12-01','AS3JSA378EWASH',100,00111111111),(00000000014,00192637485,'2020-12-05','1Z6789SADALKSDJH',100,00222222222),(00000000015,00153852934,'2020-12-09','900344465729992834702',100,00111111111),(00000000016,00643826261,'2020-12-12','1ZHJKWSLDKJASY33',100,00222222222),(00000000017,00632742987,'2020-12-15','DHAKSJD78231Q2',100,00111111111),(00000000018,00729834278,'2020-05-10','1Z0X987U3QPOWIQERU',100,00222222222);
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `inventory_management`.`incoming_goods_AFTER_UPDATE` AFTER UPDATE ON `incoming_goods` FOR EACH ROW
BEGIN
	UPDATE `inventory_management`.`current_stock`
		SET current_stock.quantity = current_stock.quantity - OLD.quantity
        WHERE current_stock.product_id = OLD.product_id;
	UPDATE `inventory_management`.`current_stock`
		SET current_stock.quantity = current_stock.quantity + NEW.quantity
        WHERE current_stock.product_id = NEW.product_id;
END */;;
...
CREATE TABLE `tracking` (
  ...
  PRIMARY KEY (`order_id`),
  CONSTRAINT `tracking_ibfk_1` FOREIGN KEY (`order_id`) REFERENCES `orders` (`order_id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
/*!40101 SET character_set_client = @saved_cs_client */;
